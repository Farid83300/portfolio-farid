...
- `Controllers/Public/{ContactController,PostController,ProjectController}.php` — `ContactController::store` saves to `messages` and best-effort emails `MAIL_TO`; `Post`/`Project` public controllers back the real public blog/portfolio pages (`index`/`show`, `Post::index` also filters by `?category=`/`?tag=` slug, both `show` methods increment `views_count`)
